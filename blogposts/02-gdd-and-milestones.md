...
**Undead Arcade** is a single-player top-down 3D twin-stick zombie shooter. The player stands in an arena, zombies attack from all sides, and the goal is to survive three waves. Last wave cleared is a win; running out of health is a loss.
...
Input is **gamepad-first** because the arcade machine has no keyboard or mouse. Unity's new Input System abstracts the hardware: the same code that reads the arcade's joystick (via Steam Input) also reads an Xbox controller plugged into a browser. A secondary **keyboard + mouse** binding (WASD to move, mouse to aim, left-click to fire) is included so the WebGL build remains playable without a controller. Gamepad is the primary design target; keyboard is a backup.

## Mechanics

- **Player:** Rigidbody-based movement. Left stick moves, right stick aims, right trigger fires continuously (rate-limited). Health bar visible at the top.
...
**In scope:** twin-stick player, one zombie type, three waves, HUD, audio, menus, animations, ScriptableObject for enemy stats, WebGL build.
